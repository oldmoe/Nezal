    ...
    module OneToMany
      ...
      def _save_one_to_many(record)
        self.class._one_to_many_hash.each_pair do | attr_name, attr_configs |        
          ...
          if attribute 
            ...
            self.class._store_one_to_many_relation(attr_name, attributes[:id], attribute.attributes[:id])
          end
          if record && record.attributes[attr_configs[:identified_by]] && ( attribute.nil? ||
            ...
            self.class._delete_one_to_many_relation( attr_name, self.attributes[:id], record.attributes[attr_configs[:identified_by]] )
          end
        end
      end
      ...
      module ClassMethods
        ...
        def _on_one_to_many_attr_delete(attr_name, attribute)
          ...
          cursor = _one_to_many_db_handle(attr_name).cursor(attribute.attributes[:id], nil, Bdb::DB_SET_RANGE)
          result = cursor.get(nil, nil, Bdb::DB_NEXT)
          while result
            if result[0] == attribute.attributes[:id]
              obj = get(result[1])
        ...
        def _store_one_to_many_relation(attr_name, id, attr_id)
          cursor = _one_to_many_db_handle(attr_name).cursor(nil, 0)
          result =  cursor.get( attr_id, id, Bdb::DB_GET_BOTH_RANGE )
          _one_to_many_db_handle(attr_name).put(nil, attr_id, id, 0) unless result == id 
        end
      
        def _delete_one_to_many_relation(attr_name, id, attr_id)
          cursor = _one_to_many_db_handle(attr_name).cursor(nil, 0)
          result =  cursor.get( attr_id, id, Bdb::DB_GET_BOTH_RANGE )
          cursor.del() if result[1] == id
        end
        
        def _one_to_many_db_handle(name)
          @_one_to_many_db_handles ||= {}
          @_one_to_many_db_handles[name] ||= Database::open(_one_to_many_table_name(name), :flags => Bdb::DB_DUP | Bdb::DB_DUPSORT)
        end
