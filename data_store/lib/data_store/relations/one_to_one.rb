    ...
    module OneToOne
      ...
      def _save_one_to_one(record)
        self.class._one_to_one_hash.each_pair do | attr_name, attr_configs |        
          ...
          self.send("_" + attr_name + "=", attribute)
      ...
      module ClassMethods
        ...
        def _on_one_to_one_attr_delete(attr_name, attribute)
          ...
          attr_conging = []
          attribute.class._one_to_one_hash.each_pair do |name, attr_configs|
            if attribute.class._one_to_one_associated_field(name) == attr_name 
              attr_config = attr_configs
            end
          end
          id = attribute._unmapped_relations[attr_config[:foreign_key]]
          if id && record = db_handler.get(id)
            record._unmapped_relations[attr_config[:foreign_key]]= nil
            record.save
          end
        end
