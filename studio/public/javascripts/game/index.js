...
var events = [
	...
	[function(){
			new Effect.Fade('loadingScreen', {duration : 1.0})
			new Effect.Appear('game')			
			//Sound.play('sounds/effects/mp3/score.mp3')
			//$('creditsScreen').show();
			//var credits = $('sms_marquee')
			//new Effect.Move(credits, { x: credits.style.width, y: 0, mode: 'relative' , duration: 25});
	}, 10],
	//[function(){SMS.scrollMessages(); $('creditsScreen').hide()}, 500],
	//[function(){SMS.scrollMessages()}, 10],
	[function(){right.speak('أهلاً بكم، سنتعرض معا طرفي مباراة جنوب أفريقيا و المكسيك')}, 20],	
	[function(){right.unspeak()}, 25],
	[function(){left.speak('لقد نسى زميلي العزيز أن يذكر أنها المباراة الافتتاحية لكأس العالم')}, 20],
	[function(){left.unspeak()}, 25],
	...
	[function(){left.speak('بل كل التوفيق للمكسيك'); right.unspeak()}, 25],
	[function(){left.unspeak()}, 25]
]
