...
var FBConnect={appIds:{"local-arabian-shield":"323491894330124"},channelPath:"xd_receiver.html",location:null,url:function(){var data=FBConnect.location;data=data.split("/");data=data[4];return data;},retry:10,callback:null,user:null,getUser:function(){FBConnect.retry--;FBConnect.callback();},init:function(successCallback){FBConnect.location=window.location.toString();var fbRoot=document.createElement('div');fbRoot.setAttribute("id","fb-root");var invite=document.createElement('iframe');invite.setAttribute("id","invite");invite.setAttribute("src","statics/blank.html");invite.style.display='none';document.body.appendChild(fbRoot);document.body.appendChild(invite);FB.init({appId:FBConnect.appIds[FBConnect.url()],apiKey:FBConnect.appIds[FBConnect.url()],status:true,cookie:true});FBConnect.callback=successCallback;FB.getLoginStatus(function(response){if(response.session){FBConnect.session=response.session;Ajax.Responders.register({onCreate:function(req){var inviter=''
...
FBConnect.appIds[FBConnect.url()]+"&canvas=1&next=http://127.0.0.1:4000/fb-games/"+
...
FBConnect.appIds[FBConnect.url()]+"&next=http://127.0.0.1:4000/fb-games/"+
FBConnect.url()+"/"+appendParam+"&display=page&locale=en_US&return_session=0&"+"fbconnect=0&canvas=1&legacy_return=1&method=permissions.request&perms=email";}
