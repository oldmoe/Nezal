var RocketLauncher = Class.create(Turret, {
	...
	render : function(){
		...
		if(this.fired){
			this.fired = false
			Game.objects.push(new Rocket(this.canvas, this.x, this.y,  {theta: this.cannonTheta, targetUnit : this.targetUnit, x : this.x, y : this.y, power: this.power}))
		}else{
		}
		this.ctx.restore();
	},
...
var Patriot = Class.create(Turret, {
	...
	render : function(){
		...
		if(this.fired){
			...
			Game.objects.push(new PatriotRocket(this.canvas, 0, 0,  {theta: this.cannonTheta, targetUnit : this.targetUnit, x : rocketX, y : rocketY, power: this.power}))
		}else{
